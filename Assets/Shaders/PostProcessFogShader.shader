...
Shader "Custom/Fog"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
			...
			float d2 = cos(IN.uv_MainTex.yx * 15 * float2(0.63243f, 0.324235f) + float2(_Time.y, _Time.y) * _NoiseTimeMult * 0.516f);
			...
			float c1 =  pow(tex2D(_FogTex, (IN.uv_MainTex ) * _FogTex_ST + delta * _FogMult).a, 2);
			float c2 = pow(tex2D(_DetailTex, (IN.uv_MainTex ) * _DetailTex_ST + delta2 * _DetailMult).a, 2);
			o.Alpha = c1 * 0.1f;
